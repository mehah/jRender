...
var Request = function(url, type, isSingleton) {
	...
	var port = JRender.WEBSOCKET_PORT || window.location.port;
	var websocket_url = "ws://" + window.location.hostname + (port ? ":" + port : "") + JRender.CONTEXT_PATH + "/coreWebSocket";
	this.send = function(p, c1, c2) {
		data = JRender.jQuery.extend(JRender.util.getQueryStrings(), p != null ? p : {});
			...
			if (!this.isWebSocket()) {
				...
				if (!(_request instanceof IframeHttpRequest)) {
					if (data instanceof Object) {
						data = JRender.jQuery.param(data);
						if (!hasContent)
	...
	this.onMessage = function(c1, c2) {
		...
		_request.onreadystatechange = function() {
			...
			if (this.readyState === UNSENT)
				...
				var txt = _request.responseText,
					isIframe = _request instanceof IframeHttpRequest,
					data = null,
					isArray = false;
