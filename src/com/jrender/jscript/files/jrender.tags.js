	...
	process: function(ref) {
		...
		for (var i = -1; ++i < listRepeat.length;) {
			...
			e.repeat = function(useOriginal, notExecuteEvent) {
				...
				var uid = JRender.cache.generateUID();
				clone.setAttribute('uid', uid);
				JRender.cache.register(uid, clone);
				...
				if (notExecuteEvent !== true)
					JRender.executeEvent('containerCloned', {
						__containerUID: uid,
						mainElement: clone
			...
			if (o != null) {
				element.registerEvent('click', function(e) {
					...
					var appendTo = this.getAttribute('appendTo'),
						...
						href = JRender.getRealURLPath(hrefOriginal),
						data = {
					...
					if (JRender.executeEvent('beforePageRequest', _data) !== false) {
						var dataComplete = "";
						...
						request.send(data, function(data) {
							...
							delete JRender.cache.tags[window.location.href];
							JRender.cache.tags[window.location.href] = tags;
								...
								if (scriptElement.src) {
									...
									script.onload = script.onreadystatechange = function() {
										if (!done && (!this.readyState || this.readyState === "loaded" || this.readyState === "complete")) {
											...
											JRender.executeEvent('scriptLoad', _data);
							...
							JRender.core.processJSON(this, o);
							...
							JRender.executeEvent('afterPageRequest', _data);
		...
		for (var i = -1; ++i < elements.length;) {
			...
			element.registerEvent('click', function() {
				var data = {},
					...
					request = new Request(this.getAttribute('action'), JRender.EVENT_REQUEST_TYPE, JRender
						.isRequestSingleton());
				...
				request.send(data, function(data) {}, function(data) {
					if (element.getAttribute('appendTo') != null) {
						...
						JRender.core.processJSON(this, element);
					} else if (data != null && data != "")
						JRender.core.processJSON(this, form, JSON.parse(data));
				});
