	...
	callRequestMethod: function(mainElement, target, event, p, __arguments) {
		...
		if (p.url[0] === '#') {
			...
			if (JRender.executeEvent('beforeEvent', _data) !== false) {
				var request = new Request(p.url, JRender.EVENT_REQUEST_TYPE, JRender.isRequestSingleton());
		...
		if (this.uidSave) {
			if (JRender.executorType.METHOD === this.type) {
				if (this.uidSave.length > 1) {
					var res = JRender.core.__isFirefox ?
						new Function('var e = arguments[0]; var mainElement = arguments[1]; return ' +
							JRender.core.commandToString(this.name, parameters)).call(this, e, mainElement) :
						eval(JRender.core.commandToString(this.name, parameters));
	...
	processJSON: function(request, mainElement, __jsonObject, argsEvent) {
		...
		for (var i in __jsonObject) {
			...
			if (jsonObject.sync != null) {
				var list = {},
					__request, url = JRender.CONTEXT_PATH + '/$synchronize';

				if (request != null && request.isWebSocket()) {
					__request = request;
					__request.setURL(url)
				} else {
					__request = new Request(url, JRender.EVENT_REQUEST_TYPE, JRender.isRequestSingleton());
				...
				for (var sI = -1; ++sI < jsonObject.sync.list.length;) {
					var sync = jsonObject.sync.list[sI],
						e = JRender.cache.getById(sync.uid, mainElement);

					if (e != null) {
						JRender.core.analizeJSON(mainElement, sync.command.parameters, e);

						var p = sync.command.parameters,
							parameters = JRender.core.analizeParameters(p, "p", mainElement),
							value, strEval = '',
							filter, forceArray = false;
		...
		for (var i = -1; ++i < res.length;) {
			var e = res[i],
				msg = JRender.internationalProperty[e.getAttribute('msg:key')];
