...
public final class ElementHandle {
	...
	public static <E extends Element> E getInstance(Class<E> clazz, Window window, Class<?> typeValue) {
		return GenericReflection.NoThrow.newInstance(clazz, new Class<?>[] { Window.class, Class.class }, window,
				typeValue);
	}
	...
	public static void dataTransfer(Element of, Element to) {
		...
		String type = DOMHandle.containVariableKey(to, "type") ? DOMHandle.getVariableValue(to, "type", String.class)
				: null;
		if (type != null)
	...
	public static Element querySelector(Element owner, String selector, HashMap<String, String[]> cssAttrs,
			boolean not) {
		Element e = new Element(owner.window);
	...
	public static Element[] querySelectorAll(Element owner, String selector, HashMap<String, String[]> cssAttrs,
			boolean not) {
		final int qnt = DOMHandle.getVariableValueByPropertyNoCache(owner, "querySelectorAll.length", Integer.class,
				"@customMethod.querySelectorAll('" + selector + "', " + new Gson().toJson(cssAttrs) + "," + not
						+ ").length");
	...
	public static Element[] querySelectorAll(Element owner, String selector, String javascriptSyntax) {
		final int qnt = DOMHandle.getVariableValueByPropertyNoCache(owner, "querySelectorAll.length", Integer.class,
				"@customMethod.querySelectorAll('" + selector + "', '" + javascriptSyntax + "').length");
