...
var Comet = function(url) {
	...
	this.jsonContentType = function(b) { if(b == null) return jsonContentType; jsonContentType = b; };
	
	this.setCometType = function(c) { cometType = c; };
	this.getCometType = function() { return cometType; };
	
	this.setCharset = function(d) { charset = d; };	
	this.getCharset = function() { return charset; };
	...
	this.forceConnectType = function(type) { forceConnectType = type; };
	...
	this.send = function(p, c1, c2)
	{
		...
		if(closed === true) {
			var
				hasContent = !(/^(?:GET|HEAD)$/.test(methodRequest)),
				parameters = "";
	...
	this.onMessage = function(c1, c2) {
		...
		Greencode.crossbrowser.registerEvent.call(window, 'unload', function () { o.abort(); });
		...
		ajaxRequest.onreadystatechange = function(){
			...
			if(this.readyState === UNSENT)
				...
				var
					...
					isIframe = ajaxRequest instanceof IframeHttpRequest,
					data = null;
