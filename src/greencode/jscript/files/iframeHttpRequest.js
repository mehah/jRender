	...
	var IframeHttpRequest = function() {
		...
		this.send = function(data) {			
			...
			for(var i in data) {
				var p = {key: i, value: data[i]};
				
				if(p.key.toLowerCase() == "submit")
					p.key = '_submit';
					
				if(Greencode.jQuery.isArray(p.value)) {
					for (var i2 in p.value)
						appendInput(form, {key: p.key+"[]", value: p.value[i2]});
				}else {
					if(p.value && p.value.nodeType && p.value.nodeType === 1 && p.value.tagName == 'INPUT' && p.value.type == 'file')
						files.push({prev: null, next: null, parent: null, e: p.value, appended: false, varName: data.varName, name: p.value.name});
					else
						appendInput(form, p);
				}
			}
			appendInput(form, {key: "isAjax", value: true});
