	...
	processJSON: function(request, mainElement, __jsonObject, argsEvent) {
		...
		for (var i in __jsonObject) {
			...
			if (jsonObject.sync != null) {
				var list = {},
					__request, url = Greencode.CONTEXT_PATH + '/$synchronize';
				...
				for (var sI = -1; ++sI < jsonObject.sync.list.length;) {
					var sync = jsonObject.sync.list[sI],
						e = Greencode.cache.getById(sync.uid, mainElement);

					if (e != null) {
						Greencode.core.analizeJSON(mainElement, sync.command.parameters, e);

						var p = sync.command.parameters,
							parameters = Greencode.core.analizeParameters(p, "p", mainElement),
							value, strEval = '',
		...
		for (var i = -1; ++i < res.length;) {
			var e = res[i],
				msg = Greencode.internationalProperty[e.getAttribute('msg:key')];
