...
public abstract class Window extends EventTarget implements HttpAction {
	...
	private Window(GreenContext context) {
		...
		greencode.jscript.$DOMHandle.setUID(this, UIDReference.WINDOW_ID.ordinal());
		...
		if(currentConversation.getAttribute("location") == null || request.isFirst() || greencode.http.$HttpRequest.contentIsHtml(request)) {
			...
			greencode.jscript.$DOMHandle.setUID(this.principalElement, UIDReference.MAIN_ELEMENT_ID.ordinal());
	...
	public boolean connectionAborted() {
		try {
			...
			if(context.getRequest().isWebSocket()) {
				...
				greencode.kernel.$DOMScanner.send(context, null);
